...
def get_relative_volatility(P_i:float, P_j:float, gamma_inf_i:float, 
    ...
    and non-ideal liquid phase. This assumption is true at low or moderate 
    pressures. In case a solvent is present, this is called pseudo-binary 
    relative volatility.
...
def get_gammas_GHGNN(gh_gnn_models, T, AD):
    T = T - 273.15 # GH-GNN uses T in C
    ln_gs = []
    for gh_gnn in gh_gnn_models:
        ln_gs.append(gh_gnn.predict(T=T, AD=AD))
    return ln_gs

def Sampling_simplex(n_features, n_samples):
    n_features = n_features - 1 # M-1
    X = sobol(n_features, n_samples)
    X_simplex = np.zeros((n_samples, n_features+1))
    
    for i in range(n_samples):
        x_sorted = [0]
        x_sorted = x_sorted + sorted(X[i])
        x_sorted.append(1)
        for j in range(1, len(x_sorted)):
            X_simplex[i,j-1] = x_sorted[j] - x_sorted[j-1]
    return X_simplex
...
def get_pseudo_binary_VLE_isothermal(c_i:dict, c_j:dict, T:float, 
    ...
    gamma_i, gamma_j, gamma_k = margules_system.get_gammas(x_i, x_j, x_k, T)
...
def get_pseudo_binary_VLE_isobaric(c_i:dict, c_j:dict, P:float, 
    ...
    for i in tqdm(range(x_i.shape[0])):
        ...
        def error_in_P(T, P_true=P):
            ...
            gamma_i, gamma_j, gamma_k = system.get_gammas(x_i[i], x_j[i], x_k[i], T)
...
def screen_with_relative_volatility(mixture, df_solv, AD):
    ...
    Ts = np.linspace(T_range[0], T_range[1], 10)
    solvents = df_solv['SMILES'].tolist()
    mixture = c_i['name'] + '_' + c_j['name']

    if not os.path.exists('results/'+mixture_type):
        os.makedirs('results/'+mixture_type)
    ...
    for t, T in enumerate(Ts):
        ...
        T_in_K = T + 273.15
        Ai, Bi, Ci = get_Antoine_constants(c_i['smiles'], T_in_K)
        P_i = get_vapor_pressure_Antoine(Ai, Bi, Ci, T_in_K)
        
        Aj, Bj, Cj = get_Antoine_constants(c_j['smiles'], T_in_K)
        P_j = get_vapor_pressure_Antoine(Aj, Bj, Cj, T_in_K)
        
        if AD is not None:
            print('\n ------ > Computing AD')
            ad_file = 'results/'+mixture_type+'/'+mixture+'_AD_'+AD+'.csv'
        ...
        for s, solvent in enumerate(tqdm(solvents)):
            if AD == 'both':
                pred_i = GH_GNN(c_i['smiles'], solvent).predict(T=T, AD=AD)
                pred_j = GH_GNN(c_j['smiles'], solvent).predict(T=T, AD=AD)
                ln_gamma_inf_i, feasible_sys_i, n_class_i, max_10_sim_i = pred_i
                ln_gamma_inf_j, feasible_sys_j, n_class_j, max_10_sim_j = pred_j 
                
                n_class_i_storage[s] = n_class_i
                n_class_j_storage[s] = n_class_j
                max_10_sim_i_storage[s] = max_10_sim_i
                max_10_sim_j_storage[s] = max_10_sim_j
                
            elif AD is None:
                ln_gamma_inf_i = GH_GNN(c_i['smiles'], solvent).predict(T=T, AD=AD)
                ln_gamma_inf_j = GH_GNN(c_j['smiles'], solvent).predict(T=T, AD=AD)
            else:
                raise Exception('Current implementation only supports AD=both or None')
        ...
        if AD is not None:
            df_AD = pd.DataFrame({
                ...
                'Solvent_name':df_solv['Name'].tolist(),
    ...
    df_results.insert(1, 'Solvent_name', df_solv['Name'].tolist())
    df_results.to_csv('results/'+mixture_type+'/'+mixture+'.csv', index=False)

def screen_with_minSF(mixture, df_solv):
    c_i = mixture['c_i']
    c_j = mixture['c_j']
    mixture_type = mixture['mixture_type']
    T_range = mixture['T_range']
    ...
    for t, T in enumerate(Ts):
        ...
        for s, solvent in enumerate(tqdm(solvents)):
            ...
            # Get relative volatility at infinite dilution
            ln_gamma_inf_i = GH_GNN(c_i['smiles'], solvent).predict(T=T, AD=None)
            ln_gamma_inf_j = GH_GNN(c_j['smiles'], solvent).predict(T=T, AD=None)
            gamma_inf_i = np.exp(ln_gamma_inf_i)
            gamma_inf_j = np.exp(ln_gamma_inf_j)
            a_inf = get_relative_volatility(P_i, P_j, gamma_inf_i, gamma_inf_j)
            ...
            if a_inf < 3:
                ...
                ln_gammas_inf = get_gammas_GHGNN(gh_gnn_models, T=T_in_K, AD=None)
                system = margules_system(*ln_gammas_inf)
                
                def Rela_Vola(SF):
                    x_i, x_j, x_k = get_xs_for_SF(SF)
                    gamma_i, gamma_j, gamma_k = system.get_gammas(x_i, x_j, x_k, T_in_K)
                    relative_volatilities = get_relative_volatility(P_i, P_j, gamma_i, gamma_j)
                    return (3 - min(relative_volatilities))**2
