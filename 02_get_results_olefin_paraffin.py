...
mixtures = ['n-hexane_1-hexene', 'n-butane_2-butene', 'n-heptane_1-heptene', 
            'n-propane_propene']
...
for mixture in mixtures:
    print('='*70)
    print(mixture)
    df = pd.read_csv(folder+'/'+mixture + '.csv')
    Ts = df.iloc[:,2:].columns.tolist()
    df_ad = pd.read_csv(folder+'/'+mixture + '_AD_both.csv')
    ...
    df_SF = pd.read_csv(folder+'/'+mixture + '_minSF.csv')
    ...
    df_feasible_SF.to_csv(folder_best+'/'+mixture + '_best_minSF.csv', index=False)
    ...
    plt.tight_layout()
    plt.close(fig)
    fig.savefig(folder_best+'/top_5_' +mixture +'_minSF.png', dpi=300, format='png')
    ...
    df_feasible.to_csv(folder_best+'/'+mixture + '_best.csv', index=False)
    ...
    plt.tight_layout()
    plt.close(fig)
    fig.savefig(folder_best+'/top_5_' +mixture +'.png', dpi=300, format='png')
    # fig.savefig(folder_best+'/parity.svg', dpi=300, format='svg')
